...
(Optionally, the modal can also create the event on your own calendar and email the
booker an invite — see *"Book it for me"* below.)
...
### "Book it for me" → create the event on your Outlook calendar (optional)

In **addition** to the add-to-your-own-calendar links above, the `/book` modal can
offer a *"have me put it on the calendar"* path: the booker enters their email,
and the Worker creates the meeting directly on **your** Microsoft 365 calendar via
**app-only Microsoft Graph** — Outlook then emails them the real invite. The
booker can optionally pick **Teams** (link generated by Graph) or **Zoom** (your
configured personal link). The slot is **re-validated server-side** against
`/slots.json` before creating (anti double-book/tamper), and the endpoint is
protected by **Cloudflare Turnstile** + a honeypot.

The whole feature is **gated by config** — without the `MS_*` secrets it's
disabled and `/book` just shows the calendar links. Setup:

1. **Entra app registration** → API permissions → **Application** permission
   `Calendars.ReadWrite` → **Grant admin consent**. Create a **client secret**.
2. **Scope it to one mailbox** (recommended) with an Exchange **Application Access
   Policy** so the app can only write to your calendar:
   ```powershell
   New-ApplicationAccessPolicy -AppId <client-id> `
     -PolicyScopeGroupId <your-mailbox-or-mail-enabled-security-group> `
     -AccessRight RestrictAccess -Description "AvailCal booking"
   ```
3. **Cloudflare Turnstile** (dash → Turnstile) → create a widget for the public
   host; note the **site key** (public) and **secret**.
4. Set secrets + vars:
   ```bash
   npx wrangler secret put MS_CLIENT_SECRET
   npx wrangler secret put TURNSTILE_SECRET
   # optional: a static personal Zoom link to enable the Zoom choice
   npx wrangler secret put ZOOM_PERSONAL_LINK
   ```
   In `wrangler.jsonc` `vars`: `MS_TENANT_ID`, `MS_CLIENT_ID`, `MS_MAILBOX`
   (UPN/primary SMTP), and `TURNSTILE_SITE_KEY`.

| Setting | Where | Meaning |
| --- | --- | --- |
| `MS_TENANT_ID` / `MS_CLIENT_ID` / `MS_MAILBOX` | vars | Entra tenant, app id, mailbox to write to |
| `MS_CLIENT_SECRET` | secret | app client secret |
| `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET` | var / secret | Turnstile widget + server verify |
| `ZOOM_PERSONAL_LINK` | secret | optional; enables the Zoom option |

Notes: the created event flows back into your busy feed (via your Outlook source →
merge), so the slot closes for others on the next cycle — there's a brief
double-book window during that lag, which the server-side re-check mostly covers.
A *unique* Zoom meeting per booking would need a separate Zoom app; the static
personal link avoids that.
